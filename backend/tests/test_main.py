...
class TestCORS:
    ...
    def test_cors_headers_on_get_request(self):
        ...
        # Make a GET request with an Origin header
    ...
    def test_cors_preflight_request(self):
        ...
        # If endpoint doesn't exist, it returns 405, but CORS headers should still be present
        assert response.status_code in [200, 405]
        # The key is that CORS headers are present even on 405
        headers_lower = {k.lower(): v for k, v in response.headers.items()}
        if "access-control-allow-origin" in headers_lower:
            assert headers_lower["access-control-allow-origin"] in [
                "*",
                "http://localhost:5173",
            ]
...
class TestAnalysisEndpoints:
    ...
    def test_analyze_endpoint_with_invalid_data(self):
        ...
        test_data = {"resume_text": "Too short", "job_description": "Also short"}
        response = client.post("/api/v1/analysis/analyze", json=test_data)
